...
#
# These are example values for a fully explicit configuration.
...
environment:
  name: prod-azlocal-01          # Short identifier used in report filenames
  clusterName: azlocal-prod-01   # Friendly cluster display name for reports
  description: Primary production Azure Local instance

targets:
  cluster:
    fqdn: azlocal-prod-01.contoso.com     # FQDN of the cluster name object (CNO)
    nodes:
      - azl-node-01.contoso.com           # At least one node FQDN or NetBIOS name
      - azl-node-02.contoso.com           # Add/remove entries to match your cluster
  azure:
    subscriptionId: 00000000-0000-0000-0000-000000000000  # Example Azure subscription ID
    resourceGroup: rg-azlocal-prod-01                     # Resource group of the Arc-enabled HCI resource
    tenantId: 11111111-1111-1111-1111-111111111111        # Example Azure AD / Entra tenant ID
  bmc:
    endpoints:
...
credentials:
  ...
  cluster:
    username: 'CONTOSO\ranger-read'       # Account with WinRM read access to cluster nodes
    passwordRef: keyvault://kv-ranger/cluster-read  # Vault reference or blank to prompt; never store plaintext
  domain:
...
output:
  mode: current-state                     # current-state | as-built
...
- **`mode`** — `current-state` for operational snapshots or `as-built` for formal handoff packages. Drift comparison is enabled with `-BaselineManifestPath`; it is not a separate output mode.
