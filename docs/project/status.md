...
## Current Release Track — v2.6.6

AzureLocalRanger v2.6.6 — Subscription-Scoped Discovery & Auth-Context Hardening — tightens how Ranger picks its target estate and orders authentication. Azure Local cluster enumeration is now explicitly scoped to the subscription ID supplied to Ranger rather than the caller's current Az context, so a session pointed at a different default subscription no longer discovers the wrong estate. Device-code, managed-identity, service-principal, certificate, and Azure CLI authentication now establish an Az context before Arc target discovery runs, and the Azure CLI path imports a subscription-scoped ARM token. Interactive runs print a non-secret resolved-target summary (tenant, subscription, resource group, cluster, nodes) before collection so operators can confirm scope up front. The setup wizard now leaves `clusterName` blank by default so Arc cluster selection can run, while current-context authentication still allows a separate cluster credential prompt. Generated YAML configuration uses discovery-friendly blank values instead of fake required placeholders and documents only the supported `current-state` and `as-built` modes. Azure/Arc and local name-resolution failures now surface actionable warnings instead of appearing only in debug logs, and the Pester unit suite migrated off the removed `Assert-MockCalled` command to `Should -Invoke` to restore green CI. v2.6.5 — Credential UX & Discovery Hardening — remains the previous release.

## Previous Release — v2.6.5 — Credential UX & Discovery Hardening
